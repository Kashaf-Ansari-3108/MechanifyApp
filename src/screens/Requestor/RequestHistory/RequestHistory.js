...
const RequestHistory = ({ navigation }) => {
    ...
    const [focused, setFocused] = useState("Completed")
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                const response = await axiosconfig.get(`requests/allRequests/${user._id}`, {
                    headers: {
            ...
            {myContext.mainLoader ? <LoadingScreen /> :
                <SafeAreaView style={{ flex: 1 }}>
                    <View style={{ flex: 1, marginBottom: moderateScale(20) }}>
                        <Header title={"Request History"} navigation={navigation} />
                        ...
                        {focused == 'Completed' ?
                            <>
                                {completedRequests.length == 0 ? <NoDataFound /> :
                                    <View style={{ width: '90%', alignSelf: 'center' }}>
                                        <FlatList
                                            ...
                                            renderItem={({ item, index }) => (
                                                <View><ReqRequestCard item={item} navigation={navigation} index={index} /></View>
                                            )}
                                        />
                                    </View>
                            ...
                            :
                            <>
                                {declinedRequests.length == 0 ? <NoDataFound /> :
                                    <View style={{ width: '90%', alignSelf: 'center' }}>
                                        <FlatList
                                            scrollEnabled={true}
                                            nestedScrollEnabled
                                            ...
                                            style={{ width: '100%' }}
                                            renderItem={({ item }) => (
                                                <View><ReqRequestCard item={item} navigation={navigation} /></View>
                                            )}
                                        />
                                    </View>
                                }
